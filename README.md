...
String (timestamp)
Timestamp (local timezone) indicating when the order is acknowledged by the restaurant.
order_ready_at
String (timestamp)
Timestamp (local timezone) indicating when the food is ready.
order_value_gbp
Float
Value of the order in GBP.
restaurant_id
Integer
...
prep_time_seconds
Integer
(order_ready_at - order_acknowledged_at) in seconds. This is the food preparation time and what you should model.
...
country
String
Country where the restaurant is
city
String
City where the restaurant is
type_of_food
String
Type of food prepared by the restaurant
